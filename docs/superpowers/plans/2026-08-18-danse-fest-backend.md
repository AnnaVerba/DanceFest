...
`userId` (UUID, null, FK → `users`, `onDelete: 'SET NULL'` — заповнюється,
коли людина завела власний акаунт).

**Індекси:** по `(lastName, firstName)` — для пошуку кандидатів у Task 8; по `userId`.
...
- [ ] **Крок 1: Падаючі тести:** (а) неавторизований запит не містить `isMine`; (б) учасник бачить `isMine: true` рівно на своїх виходах; (в) розширена проєкція для тренера → `403`; (г) виступ із перелімітом 15 с і оплатою має `overlimitSeconds: 15, overlimitPaid: true`.
...
- `GET /me/results?competitionId=` — «Мої результати»: усі виступи авторизованої
  особи в цьому конкурсі з посиланням на загальні результати (§8.13, крок 5).
...
- [ ] **Крок 5:** реалізувати сервіс і чотири ендпоінти, з тестом що стажери відсіяні.
