...
8. **Конфлікт К1 — дублікат заявки.** Якщо для цієї номінації вже існує
   неcкасована `Registration` з тим самим складом (порівняння за множиною
   `personId`) — `409`. Повідомлення **залежить від того, хто подав наявну
   заявку**, бо «дублікат» тут нічого не пояснює:
   - наявну подав тренер, подає учасник →
     `'Вас уже заявив тренер {ПІБ тренера} на цю номінацію'`;
   - наявну подав учасник, подає тренер →
     `'{ПІБ учасника} вже заявив(ла) себе на цю номінацію'`;
   - обидві від того самого автора → `'Ця заявка вже подана'`.
   У тілі відповіді разом із повідомленням віддається
   `{ existingRegistrationId }`, щоб клієнт міг показати наявну заявку.

**Правила `cancel(registrationId, userId)`:**
- Дозволено автору заявки (`submittedByUserId`), тренеру заявки (`coachId`),
  організатору й адміну конкурсу. Учасник **не може** скасувати заявку, яку
  подав тренер, і навпаки — інакше двоє знімають одне одного з конкурсу.
  Спроба → `403` з поясненням, хто може скасувати.
...
- [ ] **Крок 1: Падаючі тести:** (а) одна заявка на 3 номінації створює 3 `Registration` і 3 `Performance`; (б) обидва учасники групового номера отримали номери; (в) повторна ідентична заявка → `409`; (г) заявка після `registrationTo` → `403`; (д) помилка на третій номінації відкочує перші дві; (е) заявка від ролі `participant` має `coachId === null` і склад із власної Особи; (ж) учасник поверх заявки тренера отримує `409` з текстом про тренера і `existingRegistrationId`; (з) учасник не може скасувати заявку тренера → `403`.
...
- **Хто може вантажити:** автор заявки, тренер заявки (`coachId`), учасник, чия
  Особа входить у склад заявки, а також організатор і адмін конкурсу. Тобто
  трек може донести будь-хто з двох, хто причетний, — на відміну від скасування
  заявки (Task 6), де права розділені. Причина асиметрії: незалитий трек зриває
  виступ, а зайве скасування його скасовує.
...
- [ ] **Крок 1: Падаючі тести:** (а) wav-файл приймається; (б) `application/pdf` → `415`; (в) тривалість 72.3 с зберігається як 73; (г) завантаження на імпровізаційну заявку → `400`; (д) учасник може залити трек на заявку, яку подав його тренер; (е) сторонній тренер на чужу заявку → `403`.
...
7. `GET /competitions/:id/coach-payouts` — частка кожного тренера:
   ...
   - **до бази входять лише заявки з непорожнім `coachId`** — тобто ті, які
     подав сам тренер (рішення Р10, конфлікт К3). Заявка, яку учасник подав
...
8. **Конфлікт К2 — чий рахунок.** `GET /me/charges` віддає нарахування за
   заявками, де `submittedByUserId` — поточний користувач. Тренер бачить свої,
   учасник свої. Організатор бачить усі через `GET /competitions/:id/charges`.
