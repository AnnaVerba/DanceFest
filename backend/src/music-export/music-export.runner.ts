...
// Builds one ExportJob's .zip and uploads it to OCP, updating the row's
// status/progress as it goes. Deliberately transport-agnostic: the BullMQ
// processor and the Cloud Run Job entrypoint both just call run().
@Injectable()
export class MusicExportRunner {
  constructor(
  ...
  async run(exportJobId: string): Promise<void> {
    const exportJob = await this.exportJobModel.findByPk(exportJobId);
    if (!exportJob) return;
