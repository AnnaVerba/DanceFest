...
export class CategoryTemplatesController {
  ...
  @ApiOperation({
    summary: 'List category templates visible to the caller',
    description: 'Public templates plus the caller\'s own private ones.',
  })
  @ApiResponse({ status: 200, description: 'Templates returned.' })
  @ApiResponse({ status: 401, description: 'Missing or invalid access token.' })
  @Get()
  findAll(@CurrentUser() admin: AuthenticatedAdmin) {
    return this.categoryTemplatesService.findAll(admin.id);
  }

  @ApiOperation({ summary: 'Create a category template' })
  @ApiResponse({ status: 201, description: 'Template created; the caller becomes its author.' })
  ...
  create(
    ...
    return this.categoryTemplatesService.create(dto, admin.id);
  }

  @ApiOperation({ summary: 'Delete a category template' })
  @ApiResponse({ status: 204, description: 'Template deleted.' })
  @ApiResponse({ status: 401, description: 'Missing or invalid access token.' })
  @ApiResponse({ status: 403, description: 'Only the author can delete this template.' })
  @ApiResponse({ status: 404, description: 'No template exists with the given id.' })
  @Delete(':id')
